...
export async function generateMetadata({ params }: { params: Promise<{ puebloSlug: string }> }): Promise<Metadata> {
  ...
  const title = seoTitle(tSeo("patrimonioTitle", { nombre: puebloNombre }));
  const description = seoDescription(tSeo("patrimonioDesc", { nombre: puebloNombre }));
  return {
    title,
    description,
...
export default async function PatrimonioListPage({ params }: { params: Promise<{ puebloSlug: string }> }) {
  const { puebloSlug } = await params;
  const h = await headers();
  const locale = getLocaleFromRequestHeaders(h);

  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      ...
      <div className="mb-6 flex flex-col gap-2 border-b border-border/60 pb-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <span className="inline-flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-violet-500/10 text-lg text-violet-800 dark:bg-violet-500/15 dark:text-violet-200">
          🎬
        </span>
      </div>

      <p className="mb-6 text-sm leading-relaxed text-muted-foreground">
        Enlaces a <span className="font-medium text-foreground">YouTube</span> u otras plataformas. Se mostrarán en la ficha
        pública del pueblo.
      </p>
      ...
      {!pueblo?.id ? (
        ...
        <VideosPuebloClient slug={slug} puebloId={pueblo.id} puebloNombre={pueblo.nombre} />
      )}
    </main>
  );
}

...
export default function GestionPuebloNavidadAnterioresPage() {
  ...
  return (
    <GestionPuebloSubpageShell
      ...
      subtitle={
        <>
          Navidad · <span className="font-semibold text-white/95">{slug}</span>
        </>
      }
    ...
    >
      ...
      <div className="mb-6 rounded-xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-amber-50/40 to-red-50/30 p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-emerald-900">Tus ediciones anteriores</h2>
        <p className="mt-1 text-sm text-emerald-900/80">
          Cada Navidad queda guardada tal cual la dejasteis. Puedes consultar cualquier
          edición pasada y, si quieres, <strong>importar a la edición actual</strong> (la
          de {activeAnio ?? 'este año'}) la información, el cartel o los eventos. Las
          fechas de los eventos se trasladan automáticamente al año {activeAnio ?? 'actual'};
          tras importar podrás repasarlas y editarlas.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-emerald-300 bg-white p-8 text-center text-sm text-muted-foreground">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {ediciones.map((e) => (
            <Link
              key={e.id}
              href={`/gestion/pueblos/${slug}/navidad/anteriores/${e.anio}`}
              className="group flex items-center gap-4 rounded-xl border border-border bg-card p-4 shadow-sm transition hover:border-emerald-400 hover:shadow-md"
            >
              {e.cartelUrl ? (
                <img
                  src={e.cartelUrl}
                  alt=""
                  className="h-20 w-20 shrink-0 rounded-lg object-cover ring-1 ring-emerald-100"
                  loading="lazy"
                />
              ) : (
                <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-2xl">
                  🎄
                </div>
              )}
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-xl font-bold tracking-tight text-emerald-950">{e.anio}</span>
                ...
                <p className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-emerald-700 group-hover:text-emerald-900">
                  Ver edición

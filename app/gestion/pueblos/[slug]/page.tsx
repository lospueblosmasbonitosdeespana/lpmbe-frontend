...
/**
 * Aplana todos los PDFs de los documentos tipo MANUAL_WEB en una sola
 * lista. Cada doc puede tener un archivo principal + varios adicionales;
 * todos se muestran como ítems independientes para que el alcalde los vea
 * todos sin tener que abrir nada.
 */
function toManualItems(docs: ManualWebDoc[]): ManualItem[] {
  ...
  for (const d of docs) {
    ...
    extras.forEach((a, idx) => {
      ...
      items.push({
        ...
        url: a.url,
        // La descripción del documento padre sirve de contexto también al adjunto
        descripcion: d.descripcion,
      });
    });
  }
...
export default async function GestionPuebloPage({
  ...
  const manualesItems = toManualItems(manualesWeb);

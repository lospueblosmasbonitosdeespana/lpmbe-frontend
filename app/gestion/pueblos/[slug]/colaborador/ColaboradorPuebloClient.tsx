...
export default function ColaboradorPuebloClient({ puebloSlug }: { puebloSlug: string }) {
  ...
  if (loading) {
    return (
      <div className="p-6 text-sm text-muted-foreground">Cargando recursos...</div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-[600px]">
            ...
            <button
              ...
              className={`w-full rounded-md px-3 py-2 text-left text-sm transition-colors ${
                selectedId === r.id
                  ...
                  : 'text-muted-foreground hover:bg-white hover:text-gray-900'
              }`}
            >
      ...
      <div className="flex-1 overflow-auto">
        {!recursoActual ? (
          <div className="p-6 text-muted-foreground text-sm">Selecciona un recurso</div>
        ) : (
          <div>
            {/* Cabecera */}
            ...
            {tab === 'metricas' && (
              <div className="p-6">
                {loadingMetricas ? (
                  ...
                  <p className="text-sm text-muted-foreground">No hay datos de métricas disponibles.</p>
                ) : (
                  <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                      <div className="rounded-lg border bg-white p-4 text-center">
                        <p className="text-2xl font-bold text-blue-600">{metricas.totalValidaciones}</p>
                        <p className="text-xs text-muted-foreground mt-1">Total validaciones</p>
                      </div>
                      <div className="rounded-lg border bg-white p-4 text-center">

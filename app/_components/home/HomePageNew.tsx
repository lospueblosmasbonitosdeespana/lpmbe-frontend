    ...
    <Section spacing="lg" background="default">
      <Container>
        ...
        <div className="flex justify-center">
          <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-6 md:overflow-visible scrollbar-hide w-full max-w-6xl">
            {categories.map((cat) => (
            <Link
              key={cat.slug}
              href={cat.href}
...
function SocialMediaSection({
  ...
  const t = useTranslations("home");
  const resolvedLinks = {
    instagram: socialLinks?.instagram?.trim() || "https://www.instagram.com/lospueblosmbe/",
    facebook: socialLinks?.facebook?.trim() || "https://www.facebook.com/lospueblosmasbonitos/",
    tiktok: socialLinks?.tiktok?.trim() || "https://www.tiktok.com/@lospueblosmbe",
    youtube: socialLinks?.youtube?.trim() || "https://www.youtube.com/@lospueblosmasbonitos",
    x: socialLinks?.x?.trim() || "https://x.com/lospueblosmbe",
  };
  const socialItems = [
    {
      name: "Instagram",
      href: resolvedLinks.instagram,
      icon: (
...
/* ----- GALERÍA ACTUALIDAD ASOCIACIÓN ----- */
function AsociacionNewsGallerySection({ news = [] }: { news: NewsItem[] }) {
  const t = useTranslations("home");
  if (news.length === 0) return null;

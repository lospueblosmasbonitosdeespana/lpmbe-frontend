...
export type EdicionConfig = {
  titulo: string;
  anio: number;
};

/**
 * Ruta base para enlaces de días. Si se pasa `anio`, los enlaces apuntarán a
 * `/semana-santa/pueblo/:slug/:anio/dia/:fecha` (ediciones concretas/archivo).
 * Si no, a `/semana-santa/pueblo/:slug/dia/:fecha` (comportamiento actual).
 */
function buildDiaHref(puebloSlug: string, diaSlug: string, anio?: number): string {
  return anio
    ? `/planifica/semana-santa/pueblo/${puebloSlug}/${anio}/dia/${diaSlug}`
    : `/planifica/semana-santa/pueblo/${puebloSlug}/dia/${diaSlug}`;
}
...
interface Props {
  ...
  locale: string;
  /** Año de la edición mostrada. Si se pasa, los enlaces incluyen el año. */
  anio?: number;
  /** Si true, muestra badge "Edición archivada" en el hero (año pasado). */
  showArchivedBadge?: boolean;
}

export default async function EdicionView({
  participante,
  config,
  locale,
  anio,
  ...
  const displayAnio = anio ?? config.anio;
  ...
  return (
    <>
        ...
        <section className="relative h-[44vh] w-full overflow-hidden bg-muted">
          ...
          <div className="absolute bottom-0 left-0 right-0 mx-auto max-w-6xl px-6 pb-8 text-white">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {interesLabel && (
                <span className="inline-block rounded-full border border-white/35 bg-black/35 px-3 py-1 text-xs">
                  {interesLabel}
                </span>
              )}
              {showArchivedBadge && (
                <span className="inline-block rounded-full border border-amber-300/60 bg-amber-500/30 px-3 py-1 text-xs font-medium">
                  Edición {displayAnio} · archivo
                </span>
              )}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        <nav className="mb-6 text-sm text-muted-foreground">
          ...
          {anio && (
            <>
              ...
              <Link href={`/planifica/semana-santa/pueblo/${participante.pueblo.slug}`} className="hover:underline">
                {participante.pueblo.nombre}
              </Link>
              <span className="mx-2">/</span>
              <span>{displayAnio}</span>
            </>
          )}
        </nav>
          ...
          {diasConEventos.length === 0 ? (
            ...
            <div className="space-y-3">
              {diasConEventos.map((d) => (
                <Link key={d.id} href={buildDiaHref(participante.pueblo.slug, d.slug || d.fecha, anio)}>
        ...
        {participante.streamUrl && !showArchivedBadge && (
          <section className="mt-8 rounded-2xl border border-border bg-card p-5 shadow-sm">

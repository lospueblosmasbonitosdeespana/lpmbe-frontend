...
export async function generateMetadata({ params }: { params: Promise<{ puebloSlug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: seoDescription(`Descubre los lugares ${label.toLowerCase()} de ${puebloNombre}. Los Pueblos Más Bonitos de España.`),
...
export default async function PetfriendlyListPage({ params }: { params: Promise<{ puebloSlug: string }> }) {
  const { puebloSlug } = await params;
  const h = await headers();
  const locale = getLocaleFromRequestHeaders(h);

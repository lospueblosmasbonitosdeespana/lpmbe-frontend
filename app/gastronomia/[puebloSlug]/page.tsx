...
export async function generateMetadata({ params }: { params: Promise<{ puebloSlug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: seoDescription(`Descubre la ${label.toLowerCase()} de ${puebloNombre}. Platos típicos, recetas y restaurantes en Los Pueblos Más Bonitos de España.`),
...
export default async function GastronomiaListPage({ params }: { params: Promise<{ puebloSlug: string }> }) {
  const { puebloSlug } = await params;
  const h = await headers();
  const locale = getLocaleFromRequestHeaders(h);

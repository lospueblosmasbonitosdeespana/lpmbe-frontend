...
const ROUTE_SLUG = "donde-comprar" as const;
...
export default async function DondeComprarDetailPage({ params }: { params: Promise<{ puebloSlug: string; negocioSlug: string }> }) {
  const { puebloSlug, negocioSlug } = await params;
  const h = await headers();
  const locale = getLocaleFromRequestHeaders(h);

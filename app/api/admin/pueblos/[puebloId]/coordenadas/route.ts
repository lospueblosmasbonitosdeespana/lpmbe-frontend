...
// PATCH /admin/pueblos/:puebloId/coordenadas
export async function PATCH(
  ...
  if (!body || typeof body.lat !== 'number' || typeof body.lng !== 'number') {
    return NextResponse.json(
      { message: 'Bad Request: lat y lng son requeridos (números)' },
      { status: 400 }
    );
  }

  const API_BASE = getApiUrl();
  const upstreamUrl = `${API_BASE}/admin/pueblos/${puebloId}/coordenadas`;

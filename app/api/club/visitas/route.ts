...
export async function GET() {
  ...
  const upstreamUrl = `${API_BASE}/club/visitas`;

  if (DEV_LOGS) {
    console.error('[club/visitas] upstreamUrl:', upstreamUrl);
    console.error('[club/visitas] token exists:', !!token);
    console.error('[club/visitas] API_BASE:', API_BASE);
  }
  ...
  try {
    ...
    if (DEV_LOGS) {
      console.error('[club/visitas] fetch error:', {
        name: error?.name,
